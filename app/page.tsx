...
export default async function DashboardPage() {
  ...
  const analyticsCards = liveSnapshot?.analyticsCards ?? demoAnalyticsCards
  const segments = liveSnapshot?.segments ?? demoSegments
  const insights = liveSnapshot?.insights ?? demoInsights
  const priorityLeads = liveSnapshot?.priorityLeads ?? demoLeads
      ...
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-[#6b6560]">
            Forge GTM is centered on lead quality, enrichment coverage, pipeline movement, and segment performance so you know where to focus before sending anything.
          </p>
        </div>
        <div className="rounded-2xl border border-[#e8e4dc] bg-white px-5 py-4">
          ...
          <p className="mt-2 font-serif text-3xl font-light text-[#1a1814]">{overview.weeklyMovement}</p>
        ...
        <section className="rounded-2xl border border-[#e8e4dc] bg-white p-6">
          ...
          <div className="space-y-4">
            {segments.map((segment) => {
              const coverage = 'enrichmentCoverage' in segment ? segment.enrichmentCoverage : segment.avgScore
              const secondaryMetric = 'engagedRate' in segment ? `${segment.engagedRate}% engaged` : `${segment.velocityDays}d velocity`
              ...
              return (
                <div key={segment.name} className="rounded-xl border border-[#efeae1] bg-[#faf9f6] p-4">
                  ...
                  <div className="grid gap-3 md:grid-cols-3">
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-wide text-[#9b9589]">Momentum</p>
                      <p className="mt-1 font-serif text-xl font-light">{secondaryMetric}</p>
                    </div>
                    <div>
        ...
        <section className="rounded-2xl border border-[#e8e4dc] bg-white p-6">
          ...
          <div className="mt-6 rounded-xl border border-[#ead5c7] bg-[#fff4ed] p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-[#7a3412]">
              The app is running in single-user mode, so the fastest gains will come from improving research completeness and segment quality before scaling outbound activity.
            </p>
          </div>
        </section>
      </div>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-2xl border border-[#e8e4dc] bg-white p-6">
          <div className="mb-5">
            ...
            <h2 className="mt-1 font-serif text-2xl font-light text-[#1a1814]">Who should you work next</h2>
          </div>
        ...
        <section className="rounded-2xl border border-[#e8e4dc] bg-white p-6">
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-[#efeae1] bg-[#faf9f6] p-4">
              ...
              <p className="mt-2 font-serif text-3xl font-light">{overview.qualifiedPipeline}</p>
            ...
            <div className="rounded-xl border border-[#efeae1] bg-[#faf9f6] p-4">
              ...
              <p className="mt-2 font-serif text-3xl font-light">{overview.enrichmentCoverage}%</p>
            ...
            <div className="rounded-xl border border-[#efeae1] bg-[#faf9f6] p-4">
              ...
              <p className="mt-2 font-serif text-3xl font-light">{overview.dormantLeads}</p>
